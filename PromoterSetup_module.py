...
class Promoter_setup:

  def main(self):
    self.check_for_files()
    insider_df = pandas.read_csv(FILES['CF-Insider'])
    ...
    bhav_df = pandas.read_csv(FILES['sec_bhav'])
    ...
    working_df['BUYING QTY OF PROMOTERS/PROMOTER GROUP'] = working_df.apply(lambda row: self.get_value('purchase_qty', row['SYMBOL'], insider_df), axis=1)
    working_df['BUYING AVG PRICE OF PROMOTERS/PROMOTER GROUP'] = working_df.apply(lambda row: self.get_value('purchase_avg', row['SYMBOL'], insider_df), axis=1)
    working_df['SELLING QTY PRICE OF PROMOTERS/PROMOTER GROUP (MARKET SELL DATA)'] = working_df.apply(lambda row : self.get_value('selling_qty', row['SYMBOL'], insider_df), axis=1)
    working_df['SELLING AVG PRICE OF PROMOTERS/PROMOTER GROUP (MARKET SELL DATA)'] = working_df.apply(lambda row: self.get_value('selling_avg', row['SYMBOL'], insider_df), axis=1)

    ''' GATHER SHAREHOLDER DATA COLUMN '''
    shareholding_df = pandas.read_csv(FILES['CF-Shareholding-'])
    shareholding_df['COMPANY'] = shareholding_df['COMPANY'].str.lower()
    lookup_df = pandas.read_csv(FILES['EQUITY_L'])
    working_df['SHAREHOLDING PATTERN OF PROMOTERS/PROMOTER GROUP'] = working_df.apply(lambda row: self.get_shareholder_value(row['SYMBOL'], shareholding_df, lookup_df), axis=1)

    ''' GATHER SAST REGULATIONS COLUMN '''
    sast_reg_df = pandas.read_csv(FILES['CF-SAST- Reg'])
    working_df['SAST REGULATIONS (SOLD QTY)'] = working_df.apply(lambda row: self.get_sast_reg_value(row['SYMBOL'], sast_reg_df), axis=1)

    ''' GATHER SAST PLEDGED COLUMN '''
    sast_pledge_df = pandas.read_csv(FILES['CF-SAST-Pledged'])
    sast_pledge_df['NAME OF COMPANY'] = sast_pledge_df['NAME OF COMPANY'].str.lower()
    working_df['PLEDGED DATA OF PROMOTER/PROMOTER GROUP'] = working_df.apply(lambda row: self.get_sast_pledge_value(row['SYMBOL'], sast_pledge_df, lookup_df), axis=1)

    ''' MARK SOME TO BE GREEN FILLED BASED ON COMPLEX FILTER '''
    working_df['SIGNAL'] = working_df.apply(lambda row: self.set_signal_column_value(row), axis=1)
    ...
    working_df.to_csv("Promoter_setup_result.csv")
  ...
  # Called by lambda
  def set_signal_column_value(self,row):
  ...
  # Called by lambda
  def get_sast_pledge_value(self,symbol, sast_pledge_df, lookup_df):
    company = self.lookup_name(symbol, lookup_df)
  ...
  # Called by lambda
  def get_sast_reg_value(self,symbol, sast_reg_df):
  ...
  # Called by lambda
  def get_shareholder_value(self,symbol, shareholding_df, lookup_df):
  ...
  def lookup_name(self,symbol, lookup_df):
  ...
  # Called by lambda
  def get_value(self,mode, symbol, insider_df):
  ...
  def check_for_files(self):
    existing_files = os.listdir('.//PromoterSetupDataFiles')
    found = False
    Prefix_list = ["CF-Insider","CF-SAST- Reg","CF-SAST-Pledged","CF-Shareholding-","EQUITY_L","sec_bhav"]
    ...
    for prefix in Prefix_list:
        for existing_file in existing_files:
            if existing_file.startswith(prefix):
                FILES[prefix] = f"PromoterSetupDataFiles//{existing_file}"
                found = True
        if found is True:
            found = False
            continue
            ...
            raise EnvironmentError(
                f'Cannot find file starting with "{prefix}". Please put the file in the working directory and run the script again.')
